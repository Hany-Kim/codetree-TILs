...
void putDownToEnd(int num, PII* pos, int* dir) {
    while (1) {
        if (canMoveSouth(*pos)) {
            pos->first += 1;
        }
    ...
    map[pos->first][pos->second] = 10 * num;
    if (*dir == 2) map[pos->first + 1][pos->second] = 10 * num + 1;
    else map[pos->first + 1][pos->second] = 10 * num;

    if (*dir == 1) map[pos->first][pos->second + 1] = 10 * num + 1;
    else map[pos->first][pos->second + 1] = 10 * num;

    if (*dir == 0) map[pos->first - 1][pos->second] = 10 * num + 1;
    else map[pos->first - 1][pos->second] = 10 * num;

    if (*dir == 3) map[pos->first][pos->second - 1] = 10 * num + 1;
    else map[pos->first][pos->second - 1] = 10 * num;
...
int getOutToGolem(PII pos) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if ((map[ny][nx] / 10) != (map[now.first][now.second] / 10)) {
                if (map[now.first][now.second] % 10 == 1) {
                    used[ny][nx] = 1;
                ...
                used[ny][nx] = 1;
                maxRow = max(ny - 2, maxRow);
                if (map[ny][nx] % 10 == 1) q.push(make_pair(ny, nx));
                else if ((map[ny][nx] / 10) == (map[now.first][now.second] / 10)) q.push(make_pair(ny, nx));
            }
        }
    }
...
void sol() {
    for (int num = 1; num <= K; ++num) {
        ...
        putDownToEnd(num, &pos, &now.second); // 골렘 낙하
